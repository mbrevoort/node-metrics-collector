...
Graphite.prototype.publish = function(data, date, period) {
  console.log("in graphite publish");
  ...
  Object.keys(data).forEach(function(namespace) {
    Object.keys(data[namespace]).forEach(function(name) {
      var stat = data[namespace][name];
  ...
  this.debug && console.log(graphitePayload);
